...
const { testOrder, client } = require("./apiTest");
...
async function processBuyBuySell(buyBuySell) {
    for (let i = 0; i < buyBuySell.length; i++) {
        ...
        if (crossRate > PROFITABILITY) {
            const balance1 = await client.account()
            const beforeSald = balance1.data.balances
                .find(a => a.asset === 'USDT')
            ...
            const dataBuy1 = await testOrder(candidate.buy1.symbol, AMOUNT, "BUY");
            // console.log("ENVIANDO COMPRA 2");
            const dataBuy2 = await testOrder(candidate.buy2.symbol, 0, "BUY", dataBuy1.executedQty);
            // console.log("ENVIANDO VENDA 3");
            testOrder(candidate.sell1.symbol, dataBuy2.executedQty, "SELL");
            const balance2 = await client.account()
            const afterSald = balance2.data.balances
                .find(a => a.asset === 'USDT')
            console.log("BUY BUY CELL Concluido")
            console.log(`LUCRO: USDT ${parseFloat(beforeSaldSald.free) - parseFloat(afterSald.free)}`)
            console.log(`seu saldo atual é: ${afterSald.free}`)
...
async function processBuySellSell(buySellSell) {
    for (let i = 0; i < buySellSell.length; i++) {
        ...
        if (crossRate > PROFITABILITY) {
            ...
            const dataSell1 = await testOrder(candidate.sell1.symbol, dataBuy1.executedQty, "SELL");
            console.log("ENVIANDO VENDA 3");
            testOrder(candidate.sell2.symbol, dataSell1.executedQty, "SELL");
            const balance2 = await client.account()
            const afterSald = balance2.data.balances
                .find(a => a.asset === 'USDT')
            console.log("BUY CELL CELL Concluido")
            console.log(`LUCRO: USDT ${parseFloat(beforeSald.free) - parseFloat(afterSald.free)}`)
            console.log(`seu saldo atual é: ${afterSald.free}`)
        }
    }
}
